...
I'll reply immediately with a booking link so we can get on a call and see if we're a fit.


<form class="my-10 flex flex-wrap gap-3" data-signup-form data-placement="design-partners">
  <input
    type="email"
    name="email"
    required
    ...
    class="min-w-0 flex-[1_1_15rem] rounded-[0.6rem] border border-border bg-white px-4 py-[0.7rem] text-base text-ink transition-colors placeholder:text-ink-faint focus:border-ink focus:outline-none"
  />
  <button
    type="submit"
    class="inline-flex cursor-pointer items-center justify-center gap-2 rounded-[0.6rem] bg-ink px-5 py-[0.7rem] text-base font-medium text-surface transition-opacity hover:opacity-90 disabled:cursor-default"
  >
    <span class="btn-label">I'm interested</span>
